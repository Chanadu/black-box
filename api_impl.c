...
task_handle task_create_timeout(task_target target, time_duration duration) {
	return executor_api_task_create_timeout(target, hal_millis() + duration);
}
...
task_handle task_create_interval(task_target target, time_duration interval) {
	...
	return executor_api_task_create_interval(target, hal_millis() + interval, interval);
}
...
uint32_t debug_print(const char* str, ...) {
	...
	// because a clamped size with a sufficiently crowded stack will *also*
	// overflow
