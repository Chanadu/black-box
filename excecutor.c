...
static executor_task* allocate_task() {
	for (uint8_t task_slot = 0; task_slot < NUM_TASKS; task_slot++) {
		if (task_is(&tasks[task_slot], TASK_STATUS_ALIVE))
			continue;
...
task_handle executor_api_task_create_event(task_target target, uint32_t event_mask) {
...
task_handle executor_api_task_create_interval(task_target target,
...
task_handle executor_api_task_create_timeout(task_target target, uint32_t activate_timestamp) {
	executor_task* task = allocate_task();
	if (task == NULL)
		return 0;
...
static void debug_print_task_state() {
  ...
  for (uint8_t i=0; i<NUM_TASKS; i++) {
	...
	printf(" P=%04u a=%u b=%u\n", task->pending_activations, task->data_a,
task->data_b);
  }
...
uint32_t executor_tick_loop(uint32_t current_time, uint8_t event_counts_in[NUM_EVENTS]) {
	...
	if (current_time < last_tick_timestamp) {
		hal_panic(
			"executor_tick_loop: time went backwards! did the global timer "
			"overflow?");
	...
	for (uint8_t i = 0; i < NUM_TASKS; i++) {
		...
		if ((task->type) == TASK_TYPE_TIMEOUT) {
			...
			if (current_time >= timeout_at) {
				...
				// set the timeout to the max so it doesn't activate again before it
				// runs
...

	{
		...
		// if this task is paused (but is still in the queue from a previous
		// activation) ignore it
		if (task_is(task, TASK_STATUS_PAUSED)) {
...
done_executing:
	...
	for (uint8_t i = 0; i < NUM_TASKS; i++) {
		...
		if (!((task->type == TASK_TYPE_INTERVAL) || (task->type == TASK_TYPE_TIMEOUT)))
			continue;
